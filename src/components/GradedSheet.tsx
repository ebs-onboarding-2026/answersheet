...
/**
 * The marked-up answer sheet. The student reads their own; the author reads the
 * same sheet for any of their students, so the two views stay in step by being
...
export function GradedSheet({
  ...
  const chosenLabel = viewer === "student" ? "내가 고른 답" : "고른 답";
                    ...
                    return (
                      <li key={i} className="flex items-start gap-2.5">
                        ...
                        >
                          ...
                          {choice}
                          {isMine && !isKey ? (
                            <span className="ml-2 text-[0.8rem]">{chosenLabel}</span>
                          ) : null}
                        </span>
                      </li>
                    );
